        ...
        <section className="flex flex-row w-full justify-between gap-2">
            ...
            <div className="flex flex-col h-min bg-gray-50 shadow-sm gap-6 border flex-1 p-4 rounded">
              ...
              <p className="text-xl text-gray-700 font-bold">Summary</p>
                <p className="text-base text-gray-500 font-bold">Item count: {cartLength}</p>
              </div>

                <div>
                  <p className="text-xl text-gray-700 font-bold">Subtotal:</p>
                  <p className="text-base text-gray-500 font-bold">{cartLength * 1.25} Doggy coins DG</p>
                </div>

                <div>
                  <div className="flex flex-row justify-between">
                  <p className="text-xl text-gray-700 font-bold">Order total:</p>
                  <p className="text-base text-gray-500 font-bold">+{(cartLength * 1.25 * 0.25)} DG Tax</p>
                  </div>
                  <p className="text-base text-gray-500 font-bold">{cartLength * 1.25 + (cartLength * 1.25 * 0.25)} Doggy coins DG</p>
                </div>

                <div className="flex flex-col px-4 gap-4">
                  <button className="bg-indigo-600 px-4 py-2 rounded-md hover:bg-indigo-700 active:bg-indigo-600">
                    <p className="text-gray-50 text-xl font-bold">Proceed to checkout</p>
                  </button>

                  <Link className="bg-indigo-600 px-4 py-2 rounded-md hover:bg-indigo-700 active:bg-indigo-600" href="/">
                    <p className="text-gray-50 text-xl font-bold text-center">Continue shopping</p>
                  </Link>
                </div>
            </div>
        </section>
